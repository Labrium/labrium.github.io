...
TTA.HEIGHTFIELD_DATA_TYPE = {
  short: "short",
  float: "float"
};

TTA.createCollisionShapes = function(vertices, matrices, indexes, matrixWorld, options = {}) {
  switch (options.type) {
    case TTA.TYPE.BOX:
      return [TTA.createBoxShape(vertices, matrices, matrixWorld, options)];
    case TTA.TYPE.CYLINDER:
      return [TTA.createCylinderShape(vertices, matrices, matrixWorld, options)];
    case TTA.TYPE.CAPSULE:
      return [TTA.createCapsuleShape(vertices, matrices, matrixWorld, options)];
    case TTA.TYPE.CONE:
      return [TTA.createConeShape(vertices, matrices, matrixWorld, options)];
    case TTA.TYPE.SPHERE:
      return [TTA.createSphereShape(vertices, matrices, matrixWorld, options)];
    case TTA.TYPE.HULL:
      return [TTA.createHullShape(vertices, matrices, matrixWorld, options)];
    case TTA.TYPE.HACD:
      return TTA.createHACDShapes(vertices, matrices, indexes, matrixWorld, options);
    case TTA.TYPE.VHACD:
      return TTA.createVHACDShapes(vertices, matrices, indexes, matrixWorld, options);
    case TTA.TYPE.MESH:
      return [TTA.createTriMeshShape(vertices, matrices, indexes, matrixWorld, options)];
    case TTA.TYPE.HEIGHTFIELD:
      return [TTA.createHeightfieldTerrainShape(options)];
    default:
...
TTA.createBoxShape = function(vertices, matrices, matrixWorld, options = {}) {
  options.type = TTA.TYPE.BOX;
  _setOptions(options);

  if (options.fit === TTA.FIT.ALL) {
...
TTA.createCylinderShape = function(vertices, matrices, matrixWorld, options = {}) {
  options.type = TTA.TYPE.CYLINDER;
  _setOptions(options);
...
TTA.createCapsuleShape = function(vertices, matrices, matrixWorld, options = {}) {
  options.type = TTA.TYPE.CAPSULE;
  _setOptions(options);
...
TTA.createConeShape = function(vertices, matrices, matrixWorld, options = {}) {
  options.type = TTA.TYPE.CONE;
  _setOptions(options);
...
TTA.createSphereShape = function(vertices, matrices, matrixWorld, options = {}) {
  options.type = TTA.TYPE.SPHERE;
  _setOptions(options);

  let radius;
  if (options.fit === TTA.FIT.MANUAL && !isNaN(options.sphereRadius)) {
...
TTA.createHullShape = (function() {
  const vertex = new THREE.Vector3();
  const center = new THREE.Vector3();
  const matrix = new THREE.Matrix4();
  return function(vertices, matrices, matrixWorld, options = {}) {
    options.type = TTA.TYPE.HULL;
    _setOptions(options);

    if (options.fit === TTA.FIT.MANUAL) {
...
TTA.createVHACDShapes = (function() {
  const vector = new THREE.Vector3();
  const center = new THREE.Vector3();
  const matrix = new THREE.Matrix4();
  return function(vertices, matrices, indexes, matrixWorld, options = {}) {
    options.type = TTA.TYPE.VHACD;
    _setOptions(options);
...
TTA.createTriMeshShape = (function() {
  const va = new THREE.Vector3();
  const vb = new THREE.Vector3();
  const vc = new THREE.Vector3();
  const matrix = new THREE.Matrix4();
  return function(vertices, matrices, indexes, matrixWorld, options = {}) {
    options.type = TTA.TYPE.MESH;
    _setOptions(options);
...
TTA.createHeightfieldTerrainShape = function(options = {}) {
  _setOptions(options);
...
function _setOptions(options) {
  options.fit = options.hasOwnProperty("fit") ? options.fit : TTA.FIT.ALL;
  options.type = options.type || TTA.TYPE.HULL;
...
TTA.iterateGeometries = (function() {
  const inverse = new THREE.Matrix4();
  return function(root, options, cb) {
    root.matrixWorld.getInverse(inverse);
