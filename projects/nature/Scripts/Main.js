...
var characterLib = [
	{
		...
		sitPosition: function () {
			...
			//Mario.getObjectByName("Skl_Root_1").rotation.x += deg(-25);
			//Mario.getObjectByName("Spine2_1").rotation.z += deg(20);
			Mario.getObjectByName("ArmR_1").rotation.x += deg(-90);
			Mario.getObjectByName("ArmL_1").rotation.x += deg(90);
			Mario.getObjectByName("ElbowR_1").rotation.z += deg(-0);
			Mario.getObjectByName("ElbowL_1").rotation.z += deg(0);
			Mario.getObjectByName("ArmR_1").rotation.y += deg(-5);
			Mario.getObjectByName("ArmL_1").rotation.y += deg(5);
			Mario.getObjectByName("HandR_1").rotation.set(deg(180), deg(90), deg(90));
			Mario.getObjectByName("HandL_1").rotation.set(deg(180), deg(-90), deg(-90));
			Mario.getObjectByName("Finger1R_1").rotation.y += deg(40);
			Mario.getObjectByName("Finger1L_1").rotation.y += deg(40);
			Mario.getObjectByName("Finger2R_1").rotation.z += deg(-80);
			Mario.getObjectByName("Finger2L_1").rotation.z += deg(-80);
...
//sun
var sun = new THREE.DirectionalLight(0xffffee, 1.25);
...
lensflare.addElement(new THREE.LensflareElement(burst, 350, 0.275, new THREE.Color(0x888866)));
...
loader.load('Models/Plants/N64 tree/tree.dae', function (collada) {
	tree = collada.scene;
});
loader.load('Models/Animals/Little Bird/Bird.dae', function (collada) {
	bird = collada.scene;
});
...
function init() {
	...
	/*var groundmat = new THREE.MeshStandardMaterial({
		...
		normalMap: new THREE.TextureLoader().load('Images/grass_nrm.jpg')
	});
	groundmat.map.wrapS = THREE.RepeatWrapping;
	groundmat.map.wrapT = THREE.RepeatWrapping;
	groundmat.map.repeat.set(200, 200);
	groundmat.normalMap.wrapS = THREE.RepeatWrapping;
	groundmat.normalMap.wrapT = THREE.RepeatWrapping;
	groundmat.normalMap.repeat.set(200, 200);*/
	...
	var snow = new THREE.TextureLoader().load('Images/rock_08_diff_2k.jpg');
	snow.wrapS = THREE.RepeatWrapping;
	snow.wrapT = THREE.RepeatWrapping;
	snow.repeat.set(20, 20);

	var dirt = new THREE.TextureLoader().load('Images/forrest_ground_01_diff_2k.jpg');
	dirt.wrapS = THREE.RepeatWrapping;
	dirt.wrapT = THREE.RepeatWrapping;
	dirt.repeat.set(200, 200);

	var sand = new THREE.TextureLoader().load('Images/aerial_sand_diff_2k.jpg');
	sand.wrapS = THREE.RepeatWrapping;
	sand.wrapT = THREE.RepeatWrapping;
	sand.repeat.set(20, 20);

	var groundmat = THREE.Terrain.generateBlendedMaterial([
		{ texture: grass },
		{ texture: snow, levels: [2000, 2100, 3001, 3001] },
		{ texture: dirt, levels: [1800, 1900, 2050, 2150] },
		{ texture: sand, levels: [-250, -250, 200, 300] },
	]);
	var xS = 250, yS = 250;
	track = THREE.Terrain({
		...
		after: function (vertices, options) {
			THREE.Terrain.Edges(vertices, options, false, 2500, THREE.Terrain.EaseInOut);
		}
	});
	var test = new THREE.Object3D();
	trees = THREE.Terrain.ScatterMeshes(track.children[0].geometry, {
		mesh: test,
		w: xS,
		h: yS,
		maxSlope: deg(90),
	...
	console.log(tree);
	var newTrees = new THREE.InstancedMesh(tree.children[0].children[0].children[0].children[0].children[0].geometry, tree.children[0].children[0].children[0].children[0].children[0].material, 30000);
	var treenum = 0;
	for (var i = 0; i < track.children.length; i++) {
		//i = Math.floor(Math.random()*track.children.length);
		try {
			if (track.children[i + 1].position.z >= 4500) {
				track.children[i + 1].scale.x += (7.5);
				track.children[i + 1].scale.y += (15);
				track.children[i + 1].scale.z += (7.5);
				track.children[i + 1].translateY(-10);
				track.children[i + 1].rotateY(deg(90));
				track.children[i + 1].updateMatrix();
				newTrees.setMatrixAt(treenum, track.children[i + 1].matrix);
				treenum++;
			}
		track.children[i + 1].dispose();
		} catch (error) { }
	}
	track.add(newTrees);
	Mkart.position.y = 5000;
	console.log(newTrees);

	scene.add(bird);
	bird.scale.set(0.5,0.5,0.5);
	bird.position.set(Mkart.position.x,Mkart.position.y,Mkart.position.z);
	...
	//water
	var seaGeometry = new THREE.PlaneGeometry(100500, 100500);
	sea = new THREE.Water(seaGeometry, {
		textureWidth: 512,
		textureHeight: 512,
		waterNormals: new THREE.TextureLoader().load("Images/water nrm.png", function (texture) {
	...
	function animate() {
		...
		if (intersects[0] != undefined) {
			...
			if (height > intersects[0].distance) {
				Mkart.lookAt(new THREE.Vector3(Mkart.position.x + intersects[0].face.normal.x, Mkart.position.y + intersects[0].face.normal.z, Mkart.position.z - intersects[0].face.normal.y));
				Mkart.rotateX(deg(90));
				Mkart.rotation.y = 0;
				Mkart.rotateY(direction);
				onground = true;
				Mkart.position.y += ((height - intersects[0].distance) - 0.5);
				velocity.y = 0;
			} else {
				velocity.y -= 0.05;
				onground = false;
		...
		if (distance(bird.position, target) > 20) {
			bird.lookAt(target);
		} else {
			bird.rotateY(deg(22.5));
		}
		bird.translateZ(5.1);
		...
		sun.position.set(Mkart.position.x + 1000, Mkart.position.y + 5000, Mkart.position.z - 1000);
		//sun.position.set(Mkart.position.x, Mkart.position.y + 5000, Mkart.position.z);
